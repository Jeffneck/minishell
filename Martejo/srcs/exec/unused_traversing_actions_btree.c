...
void traverse_redir_node(t_mini *mini, t_btree *tree_el, t_io io_inherited)
{
	t_redir_fd redir;
	...
	if(!tree_el)
		return;	
	redir = search_redir_fd(tree_el->type, tree_el->content);
	browse_tree(mini, tree_el->left, io_redir_leftchild(tree_el->type, io_inherited, redir));
	close_used_redir_fd(tree_el, redir, 0); //valider le process de fermeture ici (semble utile car dans le cas de plusieurs redir successives, la commande finale n' aura pas acces aux fd qui ont ete open mais inutilises)
}

void traverse_pipe_node(t_mini *mini, t_btree *tree_el, t_io io_inherited)
{
	//se reporter a plus haut
}
...
int	main()
{
	...
	browse_tree(mini, mini->btree_root, mini->io);
}