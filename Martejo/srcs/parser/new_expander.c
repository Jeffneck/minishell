...
void	expand_dollar(t_mini *mini, t_env *env, t_token *tkn_toexpand) //cette commande ne sert pratiquement a rien, refactoriser
{
	// ft_printf(" expander_handler, token to expand = %s\n", tkn->content);
	char *dollar_lst;
	init_list(&dollar_lst);

	lstadd_dollar_expansions(dollar_lst, tkn_toexpand->content);
	if (!dollar_lst->head)
		return ;
...
void	expander(t_mini *mini,t_tknlist *tkn_lst, t_env *env)
{
	...
	{
		//expansion heredoc
		if(curr->type == WORD || curr->type == TWO_QUOTE || is_redir_tkn(curr->type))
		{
			expand_dollar(mini, env, curr, tkn_lst);
